...
from .exceptions import InternalServerError, BadRequestException, NotFoundException, UnAuthorizedException, \
    ForbiddenException, ApiGatewayException
import traceback
...
async def api_gateway_exception_handler(_: Request, exc: ApiGatewayException) -> JSONResponse:
    return await HttpResponseBuilder.build_error_response(exc.error, status_code=exc.status_code)